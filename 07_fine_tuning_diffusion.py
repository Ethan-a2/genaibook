...
dataset = load_dataset("imagefolder", data_dir="/media/code/dataset/pokemon_dataset")
...
get_ipython().system(' accelerate launch train_text_to_image.py  --pretrained_model_name_or_path="stable-diffusion-v1-5/stable-diffusion-v1-5"  @REM --dataset_name="Supermaxman/esa-hubble"  --dataset_name="reach-vb/pokemon-blip-captions"  --use_ema  --mixed_precision="fp16"  --resolution=512  --center_crop  --random_flip  --train_batch_size=1  --gradient_checkpointing  --gradient_accumulation_steps=4  --use_8bit_adam  --checkpointing_steps=1000  --num_train_epochs=50  --validation_prompts      "Hubble image of a colorful ringed nebula:  A new vibrant ring-shaped nebula was imaged by the  NASA/ESA Hubble Space Telescope."      "Pink-tinted plumes in the Large Magellanic Cloud:  The aggressively pink plumes seen in this image are extremely uncommon,  with purple-tinted currents and nebulous strands reaching out into  the surrounding space."  --validation_epochs 5  --learning_rate=1e-05  --output_dir="sd-hubble-model"')
...
prompt = (
    "a cute pokemon, pikachu, sitting on a park bench, in the style of anime, high quality, detailed"
)
